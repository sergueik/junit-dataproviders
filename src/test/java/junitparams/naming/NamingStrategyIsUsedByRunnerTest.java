...
public class NamingStrategyIsUsedByRunnerTest {
	...
	public static void checkTestCaseNames() {
		...
		String className = "("
				+ NamingStrategyIsUsedByRunnerTest.class.getCanonicalName() + ")";

		Description sampleMethodDescription = getChildDescriptionByName(
				rootDescription, "sampleMethod");

		assertThat(sampleMethodDescription.getChildren()).extracting("displayName")
				.containsExactly(
	...
	private static Description getTestClassDescription() {
		return Request.aClass(NamingStrategyIsUsedByRunnerTest.class).getRunner()
				.getDescription();
	}

	private static Description getChildDescriptionByName(Description parent,
			String expectedName) {
