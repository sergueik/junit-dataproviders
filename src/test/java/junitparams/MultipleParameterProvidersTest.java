...
public class MultipleParameterProvidersTest {
	...
	public void testWithValueAndMethodProvidersThrowsIllegalStateException() {
		Result testResult = JUnitCore
				.runClasses(CantInitializeWithValueAndMethodProvider.class);
		assertEquals(1, testResult.getFailureCount());

		Failure testFailure = testResult.getFailures().iterator().next();
		assertEquals(IllegalStateException.class,
				testFailure.getException().getClass());
		assertEquals(format(ParametersReader.ILLEGAL_STATE_EXCEPTION_MESSAGE,
				"testWithValueAndMethodProviders"), testFailure.getMessage());
	}
}
