...
public class ParamsInAnnotationTest {
	...
	public void multipleParamsCommaSeparated(int number, boolean isOne)
			throws Exception {
		if (isOne)
	...
	@Test
	@Parameters({ "1 | true", "2 | false" })
	public void multipleParamsPipeSeparated(int number, boolean isOne)
			throws Exception {
		if (isOne)
			assertThat(number).isEqualTo(1);
		else
	...
	@Test
	@Parameters({ "1, 1, 1", "1.1, 1.1, 2", "11, 11, 2", "1.11, 1.11, 3" })
	public void convertToBigDecimal(BigDecimal number, String string,
			int precision) {
